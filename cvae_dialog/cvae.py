...
parser.add_argument('--trainset_path', dest='trainset_path', default='dailydialog_src_tgt_train.tsv', type=str, help='训练集位置')
parser.add_argument('--validset_path', dest='validset_path', default='dailydialog_src_tgt_val.tsv', type=str, help='验证集位置')
parser.add_argument('--testset_path', dest='testset_path', default='dailydialog_src_tgt_test.tsv', type=str, help='测试集位置')
parser.add_argument('--embed_path', dest='embed_path', default='../glove/glove.840B.300d.txt', type=str, help='词向量位置')
...
def main():
    ...
    if args.inference:  # 测试时只载入测试集
        with open(args.testset_path, 'r', encoding='utf8') as fr:
            for line in fr:
                line = line.split('\t')
                line = {'post': line[0].split(), 'response': line[1].split()}
                testset.append(line)
